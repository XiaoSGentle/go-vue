...
func NewSlog(logPath string) *Logger {
	...
	defer func(cronLumberjack *lumberjack.Logger) {
		_ = cronLumberjack.Close()
	}(cronLumberjack)
	...
	_cronOptions := slog.HandlerOptions{
		AddSource: true,
		Level:     slog.LevelInfo,
		ReplaceAttr: func(groups []string, a slog.Attr) slog.Attr {
			if a.Key == slog.TimeKey {
	...
	return &Logger{
		...
		CronLog:  slog.New(slog.NewJSONHandler(cronLumberjack, &_cronOptions)),
	}
}
