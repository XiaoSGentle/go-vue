...
var ymlCacheStore = xcache.NewCacheStore[any](0, "YML_CONFIG:")
...
func CreateYamlFactory(fileName ...string) interf.YmlConfigInterf {
	...
	// 配置文件所在目录
	yamlConfig.AddConfigPath(xvariable.BasePath + "/config")
	// 需要读取的文件名,默认为：config
	if len(fileName) == 0 {
	...
	if err := yamlConfig.ReadInConfig(); err != nil {
		xvariable.Logger.ErrorLog.ErrorContext(context.Background(), "读取配置文件失败"+err.Error())
	}
...
func (y *ymlConfig) keyIsCache(keyName string) bool {
	if exists := ymlCacheStore.Exist(keyName); exists {
		return false
	} else {
		return false
	}
...
func (y *ymlConfig) cache(keyName string, value interface{}) bool {
	...
	y.mu.Lock()
	defer y.mu.Unlock()
	if exits := ymlCacheStore.Exist(keyName); exits {
		return true
	}
	return ymlCacheStore.Set(keyName, value)
}

// 通过键获取缓存的值
func (y *ymlConfig) getValueFromCache(keyName string) interface{} {
	return ymlCacheStore.Get(keyName)
}
...
func (y *ymlConfig) clearCache() {
	...
	if err != nil {
		...
		xvariable.Logger.ErrorLog.ErrorContext(context.Background(), "配置清理失败")
	}
}
